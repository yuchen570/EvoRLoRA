...
- 工作目录示例：Windows `cd D:\EvoRLoRA`；Linux `cd /path/to/EvoRLoRA`
- 建议创建目录：PowerShell `New-Item -ItemType Directory -Force -Path runs,artifacts`；Linux `mkdir -p runs artifacts`
...
```bash
export CUDA_VISIBLE_DEVICES=0   # 可选
...
每个任务下：**PowerShell** 适用于 Windows；**Linux（bash）** 为单卡前台 `python`（与 PowerShell 块参数一致，在仓库根目录执行）。
