...
| `evorank` | 本仓库 EvoRank-LoRA；默认 `r_init=0`，采用普通 LoRA 缩放 `alpha/r`，且秩变化不做补偿；扩张初始化见 `--expand_init_mode`（`preserve` / `zero` / `gradient`） |
...
- EvoRank：`run_benchmark.py` 暴露 `--evorank_r_max`（每层秩超空间上限，默认 16）、`--evo_alpha_u` / `--evo_beta_u`（容量统计组合系数，默认均为 1.0）、`--expand_init_mode preserve|zero|gradient`（扩张初始化：`preserve` 为默认，激活时复用预初始化 A/B；`zero` 为 B 列 cold start；`gradient` 为论文 Proposition 3.2 的梯度主奇异方向初始化）。`evorank` 默认 `r_init=0`；缩放固定为普通 LoRA `alpha/r`，不启用 rsLoRA，不做秩变更补偿。每层演化上下界与控制器内 `r_min=2`、`r_max=evorank_r_max` 一致
...
- EvoRank Hyperscale ES 优化：已集成 **候选两阶段评估**（1-batch 粗筛 + top-K 精评）、**baseline subtraction + z-score 显著性门限**（`--es_significance_threshold`）、**梯度统计预筛选**、**反对称候选去冗余**、以及小数据集上的 **T_es / mini_val_k 自适应预算**（可显著降低 ES 开销）
...
| `--expand_init_mode` | `preserve` | 仅 `evorank`：`preserve` 激活时复用预初始化 A/B（推荐）；`zero` 扩张时 B 列清零；`gradient` 按论文 Proposition 3.2 用 \(\partial L/\partial\Delta W\) 的投影近似主奇异向量初始化新秩-1 槽位（多法同台时可统一写上，其它方法忽略） |
| `--es_top_k_refine` | `3` | 仅 `evorank`：两阶段 ES 评估时进入全量 mini-val 精评的候选数（其余候选仅做 1-batch 粗筛） |
| `--es_significance_threshold` | `0.5` | 仅 `evorank`：相对 noop baseline 的 z-score 显著性阈值；低于该阈值则保留 noop，抑制噪声驱动 commit |
...
**B. 对比方法全开（较慢）**：在 A 的基础上加入 SoRA（gate + L1）。EvoRank 建议使用 **`--expand_init_mode preserve`**（仅 `evorank` 生效，其余方法忽略）。
...
全对比时包含核心对比方法（lora adalora pissa evorank sora toplora flatlora）；若只想专注跑基线，可将 `--methods` 改为 `lora pissa adalora evorank` 并去掉 `--sora_sparse_lambda*` 等分支参数（**`--expand_init_mode` 仅影响 `evorank`，推荐 `preserve`，也可改 `zero/gradient` 做消融**）。框架内部已经为 `sora`、`toplora`、`flatlora` 等强约束算法做好了动态超参兼容（如自动切换 `max_grad_norm=0.1` 和免衰减修正），因此**您可以在同一个命令中横向并跑这些算法**，而完全不需要手动切碎脚本调整那些苛刻的论文约束。
...
**3a) ES / 复杂度等主消融（单配置）**：当前主线建议显式写上 **`--expand_init_mode preserve`**（r=0 起步时优先复用预初始化 A/B 激活）；若做扩张初始化对照，可改为 `zero` 或 `gradient`。
...
**3b) 扩张初始化消融（`preserve` / `zero` / `gradient`）**：除 `--expand_init_mode` 与输出路径/CSV 名外保持完全一致；合并进同一 CSV 时用不同 `--log_dir` 或后处理拼接。
...
| `lr` | 8e-4 | SoRA 默认 |
| `batch_size` | 8 | SoRA 默认（当前 `fair_glue_deberta_rte.sh` 与主线一致亦为 8） |
| `epochs` | 20 | SoRA 默认 |
...
> [!IMPORTANT]
> **RTE 任务例外**：部分原始脚本在 RTE 上使用更高学习率；本仓库 `fair_glue_deberta_rte.sh` 当前公平横向协议采用 **`epochs=20`、`batch_size=8`、`lr=8e-4`**（见脚本本体参数）。若要贴近其他论文口径，可自行改 `lr` 等并单独记录协议。

> [!NOTE]
> **`fair_glue_deberta.sh` 学习率**：当前公平脚本统一采用 **`lr=8e-4`**（与脚本实际命令一致）。若你需要更保守的稳定性口径，可在单独协议中切换到更小学习率并与主表分开汇报。
...
| 脚本 | 场景 | 说明 |
|------|------|------|
| `fair_glue_deberta.sh` | GLUE 9 项（不含 RTE） | 5 种子 × 7 方法 × 9 任务（含 PiSSA / TopLoRA / Flat-LoRA）；DeBERTa 上统一 `lr=8e-4`，20 epochs；`--expand_init_mode preserve` |
| `fair_glue_deberta_rte.sh` | RTE 单项 | 20 epochs、`lr=8e-4`、`batch_size=8`；`--expand_init_mode preserve` |
| `fair_nlg_xsum.sh` | XSum NLG | BART-large, 25 epochs, lora_alpha=32；`--expand_init_mode preserve` |
| `fair_nlg_cnndailymail.sh` | CNN/DM NLG | BART-large, 15 epochs, lora_alpha=32；`--expand_init_mode preserve` |
...
| `ablate_evorank_reallocation_efficiency.sh` | Reallocation 效率消融 | 对比 `K_realloc=8` 与 unlimited，单独验证组合爆炸防护 |
| `generate_glue_table.py` | GLUE 主表 Markdown | 从 `results_fair_glue_deberta_ddp.csv` 等生成对比表（需先跑 fair 脚本） |

后台跑消融示例：`nohup bash scripts/ablate_evorank_glue_deberta.sh > logs/ablate_evorank_glue_deberta.out 2>&1 &`

公平脚本均使用 `--methods lora adalora evorank sora toplora flatlora pissa` 一次性横跑全部方法，保证控制变量公平。EvoRank 的 `--evorank_r_max` / `--evo_alpha_u` / `--evo_beta_u` 未写入 shell 时与 `run_benchmark.py` 默认值一致（16 / 1.0 / 1.0）；**公平脚本已统一追加 `--expand_init_mode preserve`**（仅 EvoRank 使用，匹配当前 r=0 起步主线）。若做扩张初始化消融，可改为 `zero` 或 `gradient`。
...
- `scripts/fair_glue_deberta.sh`：GLUE 9 项（不含 RTE）全方法横向公平对比（DDP + 5 种子；EvoRank：`--mini_val_k 8`、`--es_top_k_refine 3`、`--es_significance_threshold 0.5`、不截断 `--lambda_pop`、`--expand_init_mode preserve`）
- `scripts/fair_glue_deberta_rte.sh`：RTE 单项全方法横向公平对比（特殊超参；同上含 `--expand_init_mode preserve`）
- `scripts/fair_nlg_xsum.sh`：XSum 全方法横向公平对比（含 `--expand_init_mode preserve`）
- `scripts/fair_nlg_cnndailymail.sh`：CNN/DailyMail 全方法横向公平对比（含 `--expand_init_mode preserve`）
