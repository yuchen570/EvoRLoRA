...
- EvoRank：`run_benchmark.py` 暴露 `--evorank_r_max`（每层秩超空间上限，默认 16）、`--evo_alpha_u` / `--evo_beta_u`（容量统计组合系数，默认均为 1.0）；`--target_rank` 对应注入时的初始活跃秩 `r_init`。每层演化上下界还与控制器内 `r_min=2` 及 `r_max=evorank_r_max` 一致
...
| `fair_glue_deberta.sh` | GLUE 9 项（不含 RTE） | 5 种子 × 5 方法 × 9 任务；DeBERTa 上统一 `lr=2e-4`（脚本头注释说明原因），20 epochs |
| `fair_glue_deberta_rte.sh` | RTE 单项 | 50 epochs、`lr=2e-4`、`batch_size=32`（与脚本一致；原为论文级 RTE 更高 lr，此处为跨方法稳定公平） |
...
所有脚本均使用 `--methods lora adalora evorank lora-ga sora` 一次性横跑全部方法，保证控制变量公平。EvoRank 的 `--evorank_r_max` / `--evo_alpha_u` / `--evo_beta_u` 未写入 shell 时与 `run_benchmark.py` 默认值一致（16 / 1.0 / 1.0）；若消融容量统计或提高扩张 ceiling，再在命令中显式追加即可。
...
- `scripts/fair_glue_deberta.sh`：GLUE 9 项（不含 RTE）全方法横向公平对比（DDP + 5 种子；EvoRank 使用 `--mini_val_k 8`、不截断 `--lambda_pop`）
