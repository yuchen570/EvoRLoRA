...
**B. 对比方法全开（较慢）**：在 A 的基础上加入 LoRA-GA（rank0 上取少量 batch 做梯度 SVD 初始化）与 SoRA（gate + L1）。与终端冒烟一致时可为 EvoRank 打开 **`--expand_init_mode gradient`**（仅 `evorank` 生效，其余方法忽略）。
...
全对比时包含 LoRA-GA / SoRA；若只想专注跑基线，可将 `--methods` 改为 `lora adalora evorank` 并去掉 `--lora_ga_batches / --sora_sparse_lambda*` 等分支参数（**`--expand_init_mode` 仅影响 `evorank`，可保留 `gradient` 或改 `zero` 做消融**）。框架内部已经为 `sora` 和 `lora-ga` 等强约束算法做好了动态超参兼容（如自动切换 `max_grad_norm=0.1` 和免衰减修正），因此**您可以在同一个命令中横向并跑这些算法**，而完全不需要手动切碎脚本调整那些苛刻的论文约束。
...
> [!NOTE]
> **`fair_glue_deberta.sh` 学习率**：上表 SoRA 文献常用 `lr=8e-4`；DeBERTa 全 GLUE 串跑时 LoRA 在 CoLA 等任务上更易数值不稳，故公平脚本统一为 **`lr=2e-4`**，与「对比算法开源默认」表并非逐字段相同，以脚本为准。

### `scripts/` 公平对比脚本

| 脚本 | 场景 | 说明 |
|------|------|------|
| `fair_glue_deberta_smoke.sh` | SST-2 冒烟 | 50 步快速验证全方法；含 `--expand_init_mode gradient`（仅 EvoRank） |
| `fair_glue_deberta.sh` | GLUE 9 项（不含 RTE） | 5 种子 × 5 方法 × 9 任务；DeBERTa 上统一 `lr=2e-4`，20 epochs；`--expand_init_mode gradient` |
| `fair_glue_deberta_rte.sh` | RTE 单项 | 50 epochs、`lr=2e-4`、`batch_size=32`；`--expand_init_mode gradient` |
| `fair_nlg_xsum.sh` | XSum NLG | BART-large, 25 epochs, lora_alpha=32；`--expand_init_mode gradient` |
| `fair_nlg_cnndailymail.sh` | CNN/DM NLG | BART-large, 15 epochs, lora_alpha=32；`--expand_init_mode gradient` |
...
| `nohup_ablate_evorank_glue_deberta.sh` | EvoRank 主消融后台启动 | 后台启动主消融顺序实验，日志写入 `logs/ablate_evorank_glue_deberta.out` |
| `nohup_ablate_evorank_reallocation_efficiency.sh` | Reallocation 效率后台启动 | 后台启动限流效率消融，日志写入 `logs/ablate_evorank_reallocation_efficiency.out` |

所有脚本均使用 `--methods lora adalora evorank lora-ga sora` 一次性横跑全部方法，保证控制变量公平。EvoRank 的 `--evorank_r_max` / `--evo_alpha_u` / `--evo_beta_u` 未写入 shell 时与 `run_benchmark.py` 默认值一致（16 / 1.0 / 1.0）；**公平脚本已统一追加 `--expand_init_mode gradient`**（仅 EvoRank 使用，对齐论文 Proposition 3.2 的扩张初始化；消融 `zero` 时请自行改参）。若消融容量统计或提高扩张 ceiling，再在命令中显式追加即可。
...
- `scripts/fair_glue_deberta.sh`：GLUE 9 项（不含 RTE）全方法横向公平对比（DDP + 5 种子；EvoRank：`--mini_val_k 8`、不截断 `--lambda_pop`、`--expand_init_mode gradient`）
- `scripts/fair_glue_deberta_rte.sh`：RTE 单项全方法横向公平对比（特殊超参；同上含 `--expand_init_mode gradient`）
...
- `scripts/fair_glue_deberta_smoke.sh`：冒烟测试（SST-2, 50 步；含 `--expand_init_mode gradient`）
...
- `scripts/nohup_ablate_evorank_glue_deberta.sh`：后台启动主消融
- `scripts/nohup_ablate_evorank_reallocation_efficiency.sh`：后台启动 Reallocation 效率消融
...
- `scripts/summarize_evorank_ablation_templates.md`：消融表头模板
