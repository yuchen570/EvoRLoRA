...
- `run_benchmark.py`：GLUE（**HuggingFace `glue` 全部 10 个配置**）/ NLG 基准脚本，支持 LoRA、AdaLoRA、EvoRank、**SoRA**（及 MTL-LoRA 占位说明）；支持 TensorBoard / W&B、**CSV 导出**，以及 **`--output_dir` 下按实验写入 `metrics.jsonl`、可选中间 checkpoint、训练结束 `final/`（权重 + tokenizer）与 `--verify_n_samples` 冒烟打印**
...
所有脚本均使用 `--methods lora adalora evorank sora` 一次性横跑全部方法，保证控制变量公平。EvoRank 的 `--evorank_r_max` / `--evo_alpha_u` / `--evo_beta_u` 未写入 shell 时与 `run_benchmark.py` 默认值一致（16 / 1.0 / 1.0）；**公平脚本已统一追加 `--expand_init_mode gradient`**（仅 EvoRank 使用，对齐论文 Proposition 3.2 的扩张初始化；消融 `zero` 时请自行改参）。若消融容量统计或提高扩张 ceiling，再在命令中显式追加即可。
