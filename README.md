...
| 脚本 | 场景 | 说明 |
|------|------|------|
| `fair_glue_deberta.sh` | GLUE 9 项（不含 RTE） | 5 种子 × 7 方法 × 9 任务（含 PiSSA / TopLoRA / Flat-LoRA）；DeBERTa 上统一 `lr=8e-4`，20 epochs；`--expand_init_mode gradient` |
| `fair_glue_deberta_rte.sh` | RTE 单项 | 20 epochs、`lr=8e-4`、`batch_size=8`；`--expand_init_mode gradient` |
...
- `scripts/fair_glue_deberta.sh`：GLUE 9 项（不含 RTE）全方法横向公平对比（DDP + 5 种子；EvoRank：`--mini_val_k 8`、`--es_top_k_refine 3`、`--es_significance_threshold 0.5`、不截断 `--lambda_pop`、`--expand_init_mode gradient`）
